...
main (int argc, gchar * argv[])
{
  ...
  for (i = 1; i < argc; i++) {
    ...
    /* when using GStreamer as a client, one can use the gst payloader,
     * which is more efficient when there is no payloader for the
     * compressed format */
    /* g_object_set (factory, "use-gstpay", TRUE, NULL); */

    /* check if URI is valid, otherwise convert filename to URI if it's
     * a file */
    if (gst_uri_is_valid (argv[i])) {
      uri = g_strdup (argv[i]);
    ...
    /* if you want multiple clients to see the same video, set the
     * shared property to TRUE */
    gst_rtsp_media_factory_set_shared ( GST_RTSP_MEDIA_FACTORY (factory), TRUE);

    gst_rtsp_media_factory_set_retransmission_time (
        GST_RTSP_MEDIA_FACTORY (factory), 400 * GST_MSECOND);

    /* attach the test factory to the /test url */
    if (i == 1) {
      gst_rtsp_mount_points_add_factory (mounts, "/test",
          GST_RTSP_MEDIA_FACTORY (factory));
      g_print ("Attached stream at /test\n");
    } else {
      gchar *mp = g_strdup_printf ("/test%d", i-1);
      gst_rtsp_mount_points_add_factory (mounts, mp,
          GST_RTSP_MEDIA_FACTORY (factory));
      g_print ("Attached stream at %s\n", mp);
      g_free (mp);
    }
  }
  ...
  /* start serving */
  g_print ("stream(s) ready on rtsp://127.0.0.1:%s/test and /test[1 to n]\n", port);
